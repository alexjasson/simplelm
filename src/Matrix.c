...
void MatrixApply(Matrix A, EntryFunction f)
{
    ...
    ThreadPoolMap(ThreadPoolGet(), applySubset, &args, A.rows * A.cols);
}

void MatrixAdd(Matrix out, Matrix A, Matrix B)
{
    MatrixArguments args = {out, A, B};
    ThreadPoolMap(ThreadPoolGet(), addSubset, &args, A.rows * A.cols);
}

void MatrixSubtract(Matrix out, Matrix A, Matrix B)
{
    MatrixArguments args = {out, A, B};
    ThreadPoolMap(ThreadPoolGet(), subtractSubset, &args, A.rows * A.cols);
}

void MatrixHadamard(Matrix out, Matrix A, Matrix B)
{
    MatrixArguments args = {out, A, B};
    ThreadPoolMap(ThreadPoolGet(), hadamardSubset, &args, A.rows * A.cols);
}

void MatrixMultiply(Matrix out, Matrix A, Matrix B)
{
    MatrixArguments args = {out, A, B};
    ThreadPoolMap(ThreadPoolGet(), multiplySubset, &args, A.rows);
}
